        ...
        <div class="order-cont">
            ...
            <ul class="order-detail-list">
                <li class="order-detail-item" v-for="(order,index) in orderList" :key="index" @click="clickSkip(order.urlName)">
                    ...
                    <span class="order-item-num" v-text="order.orderNum"></span>
                </li>
            </ul>
        </div>
        <!-- 个人功能 -->
        <div class="other-list">
            <div class="other-item" v-for="(list,index) in listData" :key="index" @click="clickSkip(list.urlName,list.clickName)" v-if="isShow(list.isShow)">
...
export default {
    data(){
        return {
            ...
            listData: [ // 列表数据
                ...
                {name: '我的员工码',className: 'myStaffCode', urlName: 'myStaffCode.html',isShow: 'work_id'},
    ...
    methods:{
        ...
        isShow(type){
            ...
            if(type == 'work_id') return this.work_id > 0
        },
        // 点击跳转
        ...
        getUserInfo(){
            ...
            if(add){
                this.disposeData(JSON.parse(add));
            }else{
        ...
        // 获取订单数量
        getOrderNum(){
            this.dataService.orderCount().then(resp=>{
                this.dataService.handleResponse({
                    data:resp,
                    callback:(data)=>{
                        let orderList = this.orderList
                        orderList[0].orderNum = data.wait_pay;
                        orderList[1].orderNum = data.paid;
                        orderList[2].orderNum = data.shipping;
                        orderList[3].orderNum = data.wait_comment;
                        orderList[4].orderNum = data.refunding;
