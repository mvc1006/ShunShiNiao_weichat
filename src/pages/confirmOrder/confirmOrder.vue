    ...
    <div class="viewport">
		...
		<div class="shop-area" v-for="(good,index) in goods_list" :key="index">
			...
			<div class="main-shop" v-if="!share_main_order_goods_id">
				...
				<!-- 备注 -->
				<div class="shop-module shop-remark" v-if="good.goods_type==1&&!order_id">
					<div class="shop-remark-title">订单备注</div>
					<div class="shop-remark-input">
						<input type="text" maxlength="50" v-model="remark" placeholder="选填，在此处给商家留言(限50字)">
					</div>
				</div>
				<div class="shop-module shop-remark" v-else-if="good.order_goods_info && good.order_goods_info.remark">
					<div class="shop-remark-title">订单备注</div>
					<div class="shop-remark-data" v-text="good.order_goods_info.remark"></div>
				</div>
			</div>
			<!-- 赠品区域 -->
			<div class="present-shop" v-if="good.give_limit != 0">
				<!-- 赠品标题 -->
				<div class="present-shop-title" v-if="!share_main_order_goods_id">
					...
					<template v-if="good.activity_id != 3">
				...
				<div v-else class="present-shop-list">
					<div class="present-shop-item" v-for="(give,index) in good.give_list" :key="index">
						...
						<div class="delete" v-if="good.activity_id != 3">
	...
	export default {
		...
		methods:{
			...
			submit(e){
				...
				var that = this,param = this.orderData();
				if(this.commonData.ssn_shopGroup.mainShop.goods_type == 1&&!this.order_id){
		...
		computed:{
			...
			presentTotal(){
				let price = 0
				if(this.goods_list && this.goods_list[0].give_list.length > 0){
					let presentList = this.goods_list[0].give_list;
					for(let i = 0; i < presentList.length; i ++){
						price += Number(presentList[i].shop_price)
					}
				}else{
					return '0.00'
				}
