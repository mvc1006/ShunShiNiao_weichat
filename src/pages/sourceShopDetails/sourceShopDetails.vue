<template>
  <div class="viewport" :class="{'active': isJjk}">
	  	<!-- 商品组弹窗 -->
		...
		<div class="shop-data">
            ...
            <div class="serverData backColor">
                <div class="server-item" v-if="isLoacl && source_city">
                    ...
                    <div class="server-content"><span class="server-data">{{source_city}}</span><span class="server-inventory">库存剩{{shopData.store_count}}件</span></div>
                </div>
                <div class="server-item" v-if="service_title" @click="openServerData('server')">
                    <div class="server-title">服务</div>
                    <div class="server-content"><span class="server-data">{{service_title}}</span><img src="../../assets/advance.png" alt=""></div>
                </div>
                <div class="server-item" v-if="spec_info" @click="openSpe">
                    <div class="server-title">规格</div>
                    <div class="server-content"><span class="server-data">{{spec_info}}</span><img src="../../assets/advance.png" alt=""></div>
                </div>
                <div class="server-item" v-if="attr_info_string" @click="openServerData('parems')">
                    <div class="server-title">参数</div>
                    <div class="server-content"><span class="server-data">{{attr_info_string}}</span><img src="../../assets/advance.png" alt=""></div>
                </div>
            </div>
			<!-- 赠品 -->
			<template v-if="!isCityConcat && (isMainShop&&shopData.give_list&&shopData.give_list.length > 0)">
				<complimentary class="backColor-padding"
					:good="shopData"
					:is-one-live="true"
					:isJjk='isJjk'></complimentary>
		...
		<div class="merchant" v-if="is_overseas">
			...
			<div class="store-area" v-if="isLoacl">
				...
				<div class="store-data">
					...
					<div class="store-type" v-if="shop_info.shop_type != 0">
						<img v-if="shop_info.shop_type == 1" src="../../assets/flagshipStore.png" alt=""> <!-- 旗舰店 -->
						<img v-if="shop_info.shop_type == 3" src="../../assets/boutique.png" alt=""> <!-- 专营店 -->
						<img v-if="shop_info.shop_type == 2" src="../../assets/exclusiveShop.png" alt=""> <!-- 专卖店 -->
						<img v-if="shop_info.shop_type == 4" src="../../assets/optimizinGplant.png" alt=""> <!-- 工厂优选 -->
						<img v-if="shop_info.shop_type == 5" src="../../assets/venderMarket.png" alt=""> <!-- 原产地直供 -->
					</div>
					<div class="grade">
		...
		<!-- 购买 -->
		<template v-if="!isCityConcat && !isJjk">
		...
		<template v-if='isJjk && isMainShop'>
			<div class="buy-shop" @click="openSpe">立即购买</div>
		</template>
		<!-- 商品服务信息弹窗 -->
...
<script>
	...
	export default {
		...
		computed:{
			...
			// 是否为尖叫卡赠品
			isJjk(){
				return common.getUrlParam("isJjk") == 'true' ? true : false
			}
		},
      	mounted(){
		...
		methods:{
			...
			openSpe(){
				...
				if(!this.share_main_order_goods_id && JSON.stringify(mainShop) == "{}" && !this.isMainShop){
				...
				if(mainShop.goods_id == this.shopData.goods_id&&this.isMainShop) {
