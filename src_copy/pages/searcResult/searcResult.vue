	...
	<div class="viewport">
		...
		<scroll v-if="goods_list.length>0" ref="scroll" :upBack="upBack" :downBack="downBack" :isEnd="isEnd">
			<template v-if="!active_id">
				<shop-list v-if="goods_type==1" :isMainShop="isBuyPresent" :is-one-live="isOneLive" :shop-list="goods_list" :isLoacl='goods_type==1?true:false' :selectPresent='share_main_order_goods_id ? true : false'></shop-list>
				<localShopList v-if="goods_type==2" :isMainShop="isBuyPresent" :isStore="false" :shopList="goods_list" :isBuyPresent="isBuyPresent" :selectPresent='share_main_order_goods_id ? true : false'></localShopList>
			</template>
			<template v-else>
